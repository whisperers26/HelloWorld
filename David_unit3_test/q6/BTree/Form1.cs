...
namespace BTree
{
    public partial class Form1 : Form
    {
        ...
        private void Button4_Click(object sender, EventArgs e)
        {
            ...
            int[] nodes = { 1, 5, 15, 20, 21, 22, 23, 24, 25, 30, 35, 37, 40, 55, 60 };
            ...
            BTree root2 = null;
            int insertRange = nodes.Length / 2;
            //node = new BTree(nodes[nodes.Length/2], root2);
            //root2 = node;
            BalancedInsert(insertRange, insertRange);

            void BalancedInsert(int index, int range)
            {
                node = new BTree(nodelist[index], root2);
                if (root2 == null) root2 = node;
                if (range / 2 > 0)
                {
                    BalancedInsert((index - range + index-1) / 2, range / 2);
                    BalancedInsert((index + range + index+1) / 2, range / 2);
                }
                else
                {
                    node = new BTree(nodelist[index - 1], root2);
                    node = new BTree(nodelist[index + 1], root2);
                }
            }
