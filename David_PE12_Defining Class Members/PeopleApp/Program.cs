...
namespace PeopleApp
{
    class PeopleApp
    {
        //I think courses should be define here since
        //I will pass it to EditPerson to judege if the user inputs a valid course code
                    ...
                    case "list":
                        ...
                        foreach (KeyValuePair<string, Person> thisEntry in people.sortedList)
                        {
                            ...
                            if (thisPerson.GetType() == typeof(Student))
                            {
                                ...
                                {
                                    Console.WriteLine("{0} - Description for {0} {1} {2:hh:mmtt} - {3:hh:mmtt}",
                                        courseCode, string.Join(" ", courses[courseCode].schedule.daysOfWeek),
                                        courses[courseCode].schedule.startTime, courses[courseCode].schedule.endTime);
                                }
                            }
        ...
        public static void EditPerson(ref Person thisPerson)
        {
            ...
            if (thisPerson.GetType() == typeof(Student))
            {
                ...
                do
                {
                    ...
                    {
                        if (courses.sortedList.ContainsKey(sCourseCode))
                        {
                            thisStudent.courseCodes.Add(sCourseCode);
                        }
                        else
                        {
                            Console.WriteLine("Please input a valid course code!");
                        }
                    }
                    else
                    {
